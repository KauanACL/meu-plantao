...
struct AddShiftView: View {
    ...
    @State private var isAllDay: Bool = false // NOVO
        ...
        NavigationStack {
            Form {
                ...
                Section(header: Text(isCommitment ? "Título do Compromisso" : "Local do Plantão")) {
                    if isCommitment {
                        // COMPROMISSO: Campo de Texto Simples (Sem Mapa Obrigatório)
                        TextField("Ex: Aniversário, Congresso...", text: $locationName)
                            .autocorrectionDisabled()
                    } else {
                        // PLANTÃO: Busca no Mapa (Obrigatório GPS)
                        if !locationName.isEmpty {
                            HStack {
                                VStack(alignment: .leading) {
                                    Text(locationName).medFont(.headline).foregroundStyle(Color.medBlue)
                                    if latitude != nil {
                                        Text("Validado com Apple Maps").medFont(.caption).foregroundStyle(Color.secondary)
                                    }
                                }
                                Spacer()
                                Button("", systemImage: "xmark.circle.fill") {
                                    locationName = ""; latitude = nil; longitude = nil
                            ...
                            Button(action: { showLocationSearch = true }) {
                                HStack {
                                    ...
                                    Text("Buscar Hospital...").foregroundStyle(Color.secondary)
                                }
                            }
                        }
                    }
                ...
                Section(header: Text("Repetição")) {
                    Picker("Frequência", selection: $recurrence) {
                        ForEach(RecurrenceType.allCases) { type in Text(type.rawValue).tag(type) }
                    }
                    .pickerStyle(.menu)
                    
                    if recurrence == .specificDays {
                        VStack(alignment: .leading) {
                            Text("Dias da Semana").font(.caption).foregroundStyle(Color.secondary)
                            HStack {
                                ForEach([2, 3, 4, 5, 6, 7, 1], id: \.self) { day in
                                    WeekdayButton(day: day, isSelected: selectedWeekdays.contains(day)) { toggleDay(day) }
                                }
                            }
                        }.padding(.vertical, 5)
                    }
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Salvar") { saveShift() }
                        .disabled(locationName.isEmpty) // Nome é obrigatório nos dois casos
                        .fontWeight(.bold)
                }
            }
            .sheet(isPresented: $showLocationSearch) {
                LocationSearchView { name, lat, long in
                    self.locationName = name; self.latitude = lat; self.longitude = long
                }
            }
            .onAppear {
    ...
    func saveShift() {
        ...
        while loopDate <= finalDate {
            ...
            switch recurrence {
            case .none: shouldCreate = true
            case .daily: shouldCreate = true
            case .weekly: shouldCreate = true
            case .biweekly: shouldCreate = true
            case .monthly:
                let dayStart = calendar.component(.day, from: startDate)
                let dayLoop = calendar.component(.day, from: loopDate)
                if dayStart == dayLoop { shouldCreate = true }
            case .specificDays:
            ...
            if shouldCreate {
                ...
                if let shiftDate = calendar.date(from: finalComponents) {
                    let newShift = Shift(
                        startDate: shiftDate,
                        durationHours: isCommitment ? (isAllDay ? 24 : durationHours) : durationHours, // Ajuste de horas
                        locationName: locationName,
                        lat: isCommitment ? nil : latitude, // Compromisso não exige GPS
                        long: isCommitment ? nil : longitude,
                        amount: isCommitment ? 0 : amount,
                        isCommitment: isCommitment,
                        isAllDay: isAllDay // Salva a flag
                    )
...
struct WeekdayButton: View {
    let day: Int; let isSelected: Bool; let action: () -> Void
    var label: String { switch day { case 1: return "D"; case 2: return "S"; case 3: return "T"; case 4: return "Q"; case 5: return "Q"; case 6: return "S"; case 7: return "S"; default: return "" } }
    var body: some View {
        Button(action: action) {
            Text(label).font(.caption).fontWeight(.bold).frame(width: 35, height: 35)
